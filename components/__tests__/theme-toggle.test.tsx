...
describe("ThemeToggle", () => {
	...
	describe("high contrast mode", () => {
		...
		it("should use high contrast state from context", () => {
			const { container } = render(
				<ThemeProvider>
					<ThemeToggle />
				</ThemeProvider>,
			...
			// Component should successfully use the useHighContrast hook
			const button = container.querySelector("button");
			expect(button).toBeTruthy();
		});

		it("should provide setHighContrast function from context", () => {
			const { container } = render(
				<ThemeProvider>
					<ThemeToggle />
				</ThemeProvider>,
			...
			// Component should render with setHighContrast available
			expect(container.querySelector("button")).toBeTruthy();
		});

		it("should separate high contrast toggle from theme selection", () => {
			// High contrast should be a separate control from the theme dropdown items
			// This ensures users can toggle high contrast independently of their theme choice
			const { container } = render(
				<ThemeProvider>
					<ThemeToggle />
				</ThemeProvider>,
			...
			// Verify the toggle button exists (which opens dropdown with both theme and high contrast options)
			expect(container.querySelector("button")).toBeTruthy();
		});
	});
});
