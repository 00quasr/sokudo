...
import { getUser, getUserWithTeam, isUsernameTaken } from '@/lib/db/queries';
import {
  validatedAction,
...
export const signUp = validatedAction(signUpSchema, async (data, formData) => {
  const { email, password, inviteId, ref } = data;
  ...
  if (ref) {
    try {
      ...
      if (referrer && referrer.id !== createdUser.id) {
        ...
        // Check if referrer has earned a reward (e.g. Pro upgrade after 3 referrals)
        await checkAndGrantReferralReward(referrer.id);
      }
    } catch {
...
const updateAccountSchema = z.object({
  ...
  username: z
    .string()
    .max(39)
    .regex(usernameRegex, 'Username must start and end with a letter or number, and can contain hyphens and underscores')
    .optional()
    .or(z.literal('')),
});
...
export const updateAccount = validatedActionWithUser(
  ...
  async (data, _, user) => {
    ...
    if (username) {
      ...
      if (taken) {
        return { name, error: 'Username is already taken.' };
      }
    }

    await Promise.all([
      db.update(users).set({ name, email, username }).where(eq(users.id, user.id)),
