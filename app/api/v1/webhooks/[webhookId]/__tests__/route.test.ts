...
describe('GET /api/v1/webhooks/[webhookId]', () => {
  ...
  it('should return webhook with recent deliveries', async () => {
    ...
    expect(data.recentDeliveries[0].success).toBe(true);
  });
});

describe('PATCH /api/v1/webhooks/[webhookId]', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('should update webhook URL', async () => {
    ...
    const request = createMockRequest({ url: 'https://new-url.com/hook' });
    const response = await PATCH(request, { params: createParams('1') });
    const data = await response.json();

    expect(response.status).toBe(200);
    expect(data.url).toBe('https://new-url.com/hook');
  });
});
