...
export async function GET(
  ...
  try {
    const rateLimitResponse = apiRateLimit(request, { prefix: 'webhooks:get' });
...
export async function PATCH(
  ...
  try {
    const rateLimitResponse = apiRateLimit(request, { prefix: 'webhooks:update', limit: 10, windowMs: 60_000 });
    if (rateLimitResponse) return rateLimitResponse;
...
export async function DELETE(
  ...
  try {
    const rateLimitResponse = apiRateLimit(request, { prefix: 'webhooks:delete', limit: 10, windowMs: 60_000 });
    if (rateLimitResponse) return rateLimitResponse;
